...
namespace ORB_SLAM2
{
     ...
     * @return cv::Mat
     */
    cv::Mat Tracking::GrabRelocalization(const cv::Mat &img, const double timestamp)
    {
        // to do ...
        mImGray = img;
        ...
        if (bOK)
        {
            LOG(INFO)<<"relocalization ok!";
        }
        else
        {
            LOG(INFO)<<"relocalization fail!";
        }
    ...
    bool Tracking::Relocalization()
    {
        ...
        {
            LOG(INFO)<<"vpCandidateKFs is empty!";
            return false;
        }
